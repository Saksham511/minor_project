...
while True:
    ...
    for (i,c) in enumerate(counterSahpe):
        ...
        vehicle_name = classification1.classify()
        ...
        org = (x,y)
        if vehicle_name == 0:
            cv2.putText(frame1, 'bike', org, font, fontScale, color, thickness, cv2.LINE_AA)
        elif vehicle_name == 1:
            cv2.putText(frame1, 'car', org, font, fontScale, color, thickness, cv2.LINE_AA)
        else:
            cv2.putText(frame1, 'truck', org, font, fontScale, color, thickness, cv2.LINE_AA)
